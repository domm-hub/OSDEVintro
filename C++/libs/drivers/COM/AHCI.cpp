...
namespace AHCI {
    ...
    bool Port::Read(uint_64 sector, uint_32 sectorCount, void* buffer) {
        ...
        // 1. Clear Error Status by writing all 1s
        ...
        if (spin == 1000000) {
             if (GlobalRenderer) GlobalRenderer->Print("TFD Hang\n");
             return false;
        }
        ...
        while (1) {
            ...
            if (hbaPort->is & (1 << 30)) {
                if (GlobalRenderer) {
                    GlobalRenderer->Print("AHCI ERR IS: ");
                    GlobalRenderer->Print(HexToString(hbaPort->is));
                    GlobalRenderer->Print(" SERR: ");
                    GlobalRenderer->Print(HexToString(hbaPort->serr));
    ...
    Driver::Driver(uint_16 bus, uint_16 slot, uint_16 func) {
        ...
        if ((bar5 & 0x06) == 0x04) {
            uint_32 bar_upper = PCI::ReadDWord(bus, slot, func, 0x28);
            abar_phys |= ((uint_64)bar_upper << 32);
        }
            ...
            for (uint_16 slot = 0; slot < 32; slot++) {
                for (uint_16 func = 0; func < 8; func++) {
                    ...
                    if (classID == 0x01 && subclassID == 0x06) {
                        ...
                        if (GlobalAHCIDriver) {
                            new (GlobalAHCIDriver) Driver(bus, slot, func);
                        }
                        return;
                    }
                }
